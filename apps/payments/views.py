...
def initiate_checkout(request, slug):
    ...
    order = _client().order.create({'amount': amount_paise, 'currency': 'INR', 'payment_capture': 1})
    return render(request, 'payments/checkout.html', {
        'event': event, 'razorpay_order_id': order['id'],
        'razorpay_key_id': settings.RAZORPAY_KEY_ID,
        'amount_paise': amount_paise, 'amount_display': event.ticket_price,
        'user_name': request.user.get_full_name() or request.user.username,
        'user_email': request.user.email, 'user_phone': getattr(request.user, 'phone', '') or '',
    })
...
def payment_success(request):
    ...
    Payment.objects.create(
        ticket=ticket, user=request.user, amount=event.ticket_price,
        status=Payment.Status.SUCCESS, razorpay_order_id=order_id,
        razorpay_payment_id=payment_id, razorpay_signature=signature, created_by=request.user,
    )
    try:
...
def razorpay_webhook(request):
    ...
    expected = hmac.new(settings.RAZORPAY_KEY_SECRET.encode(), request.body, hashlib.sha256).hexdigest()
...
def _create_free_ticket(request, event):
    ticket = Ticket.objects.create(
        event=event, attendee=request.user,
        ticket_code=f'TKT-{uuid.uuid4().hex[:8].upper()}',
        status='confirmed', amount_paid=0, created_by=request.user,
    )
